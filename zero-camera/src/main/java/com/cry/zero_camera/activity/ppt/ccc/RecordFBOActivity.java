...
package com.cry.zero_camera.activity.ppt.ccc;
...
import com.cry.zero_camera.activity.ppt.render.AnimateFilter;
import com.cry.zero_camera.ref.MiscUtils;
import com.cry.zero_camera.ref.VideoEncoderCore;
import com.cry.zero_camera.ref.gles.Drawable2d;
import com.cry.zero_camera.ref.gles.EglCore;
import com.cry.zero_camera.ref.gles.FlatShadedProgram;
import com.cry.zero_camera.ref.gles.FullFrameRect;
import com.cry.zero_camera.ref.gles.GlUtil;
import com.cry.zero_camera.ref.gles.Sprite2d;
import com.cry.zero_camera.ref.gles.Texture2dProgram;
...
public class RecordFBOActivity extends Activity implements SurfaceHolder.Callback,
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        File outputFile = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES), "fbo-gl-recording" + System.currentTimeMillis() + ".mp4");
        SurfaceView sv = (SurfaceView) findViewById(R.id.fboActivity_surfaceView);
    ...
    public void onRadioButtonClicked(View view) {
        ...
        int i = rb.getId();
        if (i == R.id.recDrawTwice_radio) {
            mSelectedRecordMethod = RECMETHOD_DRAW_TWICE;

        } else if (i == R.id.recFbo_radio) {
            mSelectedRecordMethod = RECMETHOD_FBO;

        } else if (i == R.id.recFramebuffer_radio) {
...
//            mVideoRect.set(0,0,windowWidth,windowHeight);
        ...
        private void update(long timeStampNanos) {
            ...
            float[] srcMatrix = filter.getSrcMatrix();
            if (srcMatrix == null) {
                return;
            }
            float[] floats = Arrays.copyOf(srcMatrix, 16);
            float scale = (float) (filter.getScale() + 0.01 * 0.5);
            if (scale > 1.5) {
                scale = 1;
            }
            Matrix.scaleM(floats, 0, scale, scale, 0f);
            filter.setScale(scale);
            filter.setAnimateMatrix(floats);
        }
...
//            }
        ...
        public void setBitmap(Bitmap obj) {
            filter.setBitmap(obj);
        }
    }
    ...
    private static class RenderHandler extends Handler {
        ...
        public void sendBitmap(Bitmap bitmap) {
            sendMessage(obtainMessage(RenderHandler.MSG_BITMAP, bitmap));
        }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (what) {
                ...
                case MSG_SHUTDOWN:
                    ...
                    break;
                case MSG_BITMAP:
                    renderThread.setBitmap((Bitmap) msg.obj);
                    break;
                default:
